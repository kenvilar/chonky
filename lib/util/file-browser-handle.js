...
exports.useFileBrowserHandle = function (ref) {
    var store = react_redux_1.useStore();
    var dispatch = react_redux_1.useDispatch();
    react_1.useImperativeHandle(ref, function () { return ({
        getFileSelection: function () {
            var selectionMap = selectors_1.selectSelectionMap(store.getState());
        ...
        requestFileAction: function (action, payload) {
            return Promise.resolve(dispatch(dispatchers_thunks_1.thunkRequestFileAction(action, payload))).then();
        },
    }); }, [store, dispatch]);
};
