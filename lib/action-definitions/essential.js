...
exports.EssentialActions = {
     ...
     */
    MouseClickFile: helpers_1.defineFileAction({
        id: 'mouse_click_file',
        __payloadType: {},
    }, function (_a) {
        var _b;
        ...
        if (payload.clickType === 'double') {
            if (file_helper_1.FileHelper.isOpenable(payload.file)) {
                reduxDispatch(dispatchers_thunks_1.thunkRequestFileAction(index_1.ChonkyActions.OpenFiles, {
            ...
            var disableSelection = selectors_1.selectDisableSelection(getReduxState());
            if (file_helper_1.FileHelper.isSelectable(payload.file) && !disableSelection) {
                if (payload.ctrlKey) {
                    ...
                    // Range selection
                    var lastClickIndex = selectors_1.selectLastClickIndex(getReduxState());
     ...
     */
    KeyboardClickFile: helpers_1.defineFileAction({
        ...
        if (payload.enterKey) {
            ...
            if (selectors_1.selectSelectionSize(getReduxState()) === 0) {
                reduxDispatch(dispatchers_thunks_1.thunkRequestFileAction(index_1.ChonkyActions.OpenFiles, {
                    targetFile: payload.file,
                    files: [payload.file],
                }));
     ...
     */
    StartDragNDrop: helpers_1.defineFileAction({
        id: 'start_drag_n_drop',
        __payloadType: {},
    }, function (_a) {
        var payload = _a.payload, reduxDispatch = _a.reduxDispatch, getReduxState = _a.getReduxState;
        var file = payload.draggedFile;
        if (!selectors_1.getIsFileSelected(getReduxState(), file)) {
     ...
     */
    EndDragNDrop: helpers_1.defineFileAction({
        id: 'end_drag_n_drop',
        __payloadType: {},
    }, function (_a) {
        var payload = _a.payload, reduxDispatch = _a.reduxDispatch, getReduxState = _a.getReduxState;
        if (selectors_1.getIsFileSelected(getReduxState(), payload.destination)) {
        ...
        reduxDispatch(dispatchers_thunks_1.thunkRequestFileAction(index_1.ChonkyActions.MoveFiles, __assign(__assign({}, payload), { files: droppedFiles })));
    }),
    /**
     ...
     */
    MoveFiles: helpers_1.defineFileAction({
        id: 'move_files',
        __payloadType: {},
    }),
    /**
     ...
     */
    ChangeSelection: helpers_1.defineFileAction({
        id: 'change_selection',
        __payloadType: {},
    }),
    /**
     ...
     */
    OpenFiles: helpers_1.defineFileAction({
        id: 'open_files',
        __payloadType: {},
    }),
    /**
     ...
     */
    OpenParentFolder: helpers_1.defineFileAction({
        ...
        var parentFolder = selectors_1.selectParentFolder(getReduxState());
     ...
     */
    OpenFileContextMenu: helpers_1.defineFileAction({
        ...
        var triggerFile = selectors_1.getFileData(getReduxState(), payload.triggerFileId);
        if (triggerFile) {
            var fileSelected = selectors_1.getIsFileSelected(getReduxState(), triggerFile);
            if (!fileSelected) {
