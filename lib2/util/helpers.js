...
exports.defineFileAction = function (action, effect) {
    if (action.__payloadType !== undefined && (action.hotkeys || action.button)) {
        var errorMessage = "Invalid definition was provided for file action \"" + action.id + "\". Actions " +
 ...
 */
exports.findElementAmongAncestors = function (maybeElement, predicate) {
    if (!maybeElement)
        return maybeElement;
    if (predicate(maybeElement))
        return maybeElement;
    if (maybeElement.parentElement) {
        return exports.findElementAmongAncestors(maybeElement.parentElement, predicate);
    }
    return null;
};
exports.elementIsInsideButton = function (buttonCandidate) {
    return !!exports.findElementAmongAncestors(buttonCandidate, function (element) { return element.tagName && element.tagName.toLowerCase() === 'button'; });
};
exports.getValueOrFallback = function (value, fallback, desiredType) {
    if (desiredType) {
